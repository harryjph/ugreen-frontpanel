...
#define AXS_DRIVER_VERSION                  "V2.1.7"
...
#define AXS_REG_VERSION_READ            0X0C        /*﹞米??firware～?㊣?*/
//#define AXS_REG_GESTURE_READ          0X0D        /*﹞米??那?那?那1?邦*/
#define AXS_REG_GESTURE_WRITE           0X23        /*谷豕??那?那?那1?邦*/
...
// tp exception code
#define RAWDATA_EXCEPTION 0xa0
#define SCAN_EXCEPTION 0xb0
#define ESD_EXCEPTION 0xc0
...
//  struct mutex report_mutex;
    ...
    u8 esd_host_enable;
	u8 esd_firmware_enable;
	u8 esd_firmware_enable_delay;
	u8 esd_firmware_enable_fail_count;
    ...
    u8 *debug_rx_buf;
    int bus_type;
    u8 point_buf[15];/*AXS_MAX_TOUCH_NUMBER*AXS_ONE_TCH_LEN+3*/
    u16 pnt_buf_size;
    int touchs;
    int key_state;
    int touch_point;
...
bool axs_read_gesture_enable(u8 *enable);
bool axs_write_gesture_enable(void);
#endif
